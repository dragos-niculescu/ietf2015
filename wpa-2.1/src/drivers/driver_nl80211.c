...
static int bss_info_handler(struct nl_msg *msg, void *arg)
{
	...
	//DRAGOS - si aici se cauta duplicate 
	/*
...
static int nl80211_addr_in_use_assoc(struct nl80211_global *global, const u8 *addr)
{
	struct wpa_driver_nl80211_data *drv;
	dl_list_for_each(drv, &global->interfaces,
			 struct wpa_driver_nl80211_data, list) {
	  if(drv->associated){
	    //wpa_printf(MSG_INFO, MACSTR "=" MACSTR, MAC2STR(addr), MAC2STR(drv->bssid));  
...
// SSID vine în formatul ăsta \x00\x07ccna.ro\x??...
// formatăm la stil C / zero terminated
 char * pascal_string(const u8* ssid, int len)
 {
   static char c_str[256];
   if(len > 255)
     len = 255; 
   
   memcpy(c_str, ssid+2, ssid[1]); 
   c_str[ssid[1]] = 0; 
   return c_str; 
  }

void delete_used_bssids(struct wpa_driver_nl80211_data *drv, struct wpa_scan_results *res)
{
  int i;
  for(i = 0; i < res->num; i++){
    int used = nl80211_addr_in_use_assoc(drv->global, res->res[i]->bssid); 
    const u8 *ssid = nl80211_get_ie((u8 *) (res->res[i] + 1),
				    res->res[i]->ie_len, WLAN_EID_SSID);
    wpa_printf(MSG_INFO, "DRAGOS %s " MACSTR " %-32s %d used:%d", 
	       drv->first_bss->ifname, 
	       MAC2STR(res->res[i]->bssid), 
	       pascal_string(ssid, res->res[i]->ie_len),
	       res->res[i]->level, 
	       used);
    if(used){
      memset(res->res[i], 0, sizeof(struct wpa_scan_res)); 
      res->res[i]->level = -200; 
      res->res[i]->noise = 200; 
      res->res[i]->qual = -200; 
      // maybe we should just delete the entry
    }
  }
}
...
nl80211_get_scan_results(struct wpa_driver_nl80211_data *drv)
{
	...
	msg = NULL;
	if (ret == 0) {
	        delete_used_bssids(drv, res); 
		wpa_printf(MSG_INFO, "DRAGOS nl80211: Received scan results (%lu "
